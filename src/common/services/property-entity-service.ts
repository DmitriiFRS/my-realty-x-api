...
@Injectable()
export abstract class PropertyEntityService<TModel, TCreateDto extends PropertyEntityCreateDto> {
  constructor(
  ...
  async findMany(locale: string, include?: any): Promise<TModel[]> {
    try {
      ...
      throw new BadRequestException(`Failed to fetch ${this.modelName}: ${error.message}`);
    }
  }

  async create(dto: TCreateDto): Promise<TModel[]> {
    if (!dto || !dto.slug || !dto.translations?.length) {
      throw new BadRequestException('Slug and at least one translation are required');
    }

    const defaultTranslation = dto.translations.find((t) => t.locale === 'ru');

    if (!defaultTranslation) {
      throw new BadRequestException('Default translation for "ru" locale is required.');
    }
    ...
    try {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
        throw new BadRequestException(`A ${this.modelName} with this slug already exists.`);
      }
